...
The template, a worked example, and the test-first (RED → GREEN) rules for
`plan-T-NNN-*.md`. Plans are untracked working artifacts under
`.kodit/tmp/specs/`; they attach to a task and live only for its work.
...
plan — amend with a new plan or a dated entry, following `AGENTS.md`.
