...
Run every `linear` command with the `terminal` tool. If `linear` is not on
PATH, run it as `npx @schpet/linear-cli`. The `linear-cli` skill documents
installation, authentication, and the full command surface; load it with
`skill_view` when available.
...
- **PR URL:** stored in the milestone's **description** as `**PR:** <url>`,
  written with `linear milestone update <id> --description "<description>"`
  (or a `milestoneUpdate` GraphQL mutation, which takes the full markdown when
  the CLI's inline `--description` is awkward). Idempotent write; no status
  change.
...
- Record PR / merge metadata: update the milestone **description**
  (`linear milestone update <id> --description "<description>"`), per the format
  in "PR and merge metadata" above. Never record it as a comment on the
  milestone.
