...
| 2026-09-26 | Skill authoring is scoped to this repository: create and edit skills only under `skills/` or `hermes-skills/`, never in agent config or install directories (`~/.config/opencode/skills/`, `~/.hermes/skills/`, `~/.claude/skills/`). | The repository is the source of truth for skills; writing directly into install paths bypasses review, creates untracked drift, and makes changes unreproducible. Installing a skill onto a machine is a distribution step, not authoring, and is handled by the taps/publish flow in `hermes-skills/AGENTS.md`. |
| 2026-09-26 | Hermes skills express command execution as Hermes tool calls (`terminal(command="...")`, `process(action=...)`) rather than bare shell code fences. | The official Hermes skills (e.g. `autonomous-ai-agents/opencode`) use this notation, so it is the house style agents are trained on. Naming the tool removes the ambiguity of an assumed shell and makes every example directly executable by the target runtime. |
| 2026-09-26 | Add a `tools` category under `hermes-skills/` for skills that wrap a single external command-line tool, and move `gh-cli` and `linear-cli` there from `general/`. | These skills are thin, CLI-shaped wrappers around a third-party tool (`gh`, `linear`) rather than general capabilities, so grouping them keeps `general/` for reusable, phase-agnostic capabilities. Skill frontmatter names and directory names are unchanged (only the category path moves), so the name-equals-directory invariant holds. Distribution paths change to `hermes-skills/tools/NAME`; tap-style category recursion remains unverified. |
| 2026-09-27 | Hermes skills are distributed by direct install and tap from the repository's default branch; there is no publish or release step for `hermes-skills/`. | `hermes skills install OWNER/REPO/hermes-skills/<category>/NAME` reads the default branch tree directly, and install never consults `taps.json`; tags and GitHub Releases are not used by the Hermes GitHub source at all. The `hermes skills publish` command forks a *target* repo and opens a PR putting files under that repo's `skills/` directory, so it is a contribution flow for other repositories and cannot publish this one. Merging to `master` is therefore the whole distribution act, and CI's job is to guarantee validity rather than to publish. |
| 2026-09-27 | Gate `hermes-skills/` in CI with `.github/scripts/validate-hermes-skills.py` (frontmatter parses as YAML, `name` is lowercase kebab-case and equals its directory, `description` is present and ≤ 1024 chars), run by both `validate-skills.yml` and `release-skills.yml`. | `gh skill publish --dry-run` cannot see this tree: its discovery conventions (`skills/*`, `skills/{scope}/*`, `*/SKILL.md`, `plugins/{scope}/skills/*`) never match `hermes-skills/<category>/<name>/SKILL.md`, and pointing it at the subtree finds zero skills and errors. One shared script keeps the rule single-sourced across the two workflows instead of duplicating logic inline. It is CI-only tooling, not runtime code for adopting projects, and it enforces the frontmatter invariants already stated in `hermes-skills/AGENTS.md`; the nested `skill-creator/assets/skill-template/SKILL.md` asset is excluded because only immediate category children are scanned. |
| 2026-09-27 | Correct the Distribution section of `hermes-skills/AGENTS.md`: document per-category taps, the default-branch-only install resolution, and drop the `hermes skills publish` instruction. | Tap discovery walks exactly one level under the tap path, so a single tap with `path: "hermes-skills/"` provably discovers nothing — the earlier note left this "unverified" while the underlying behavior is confirmed in the Hermes source and docs. Each category needs its own hand-edited `taps.json` entry, because `hermes skills tap add` hardcodes `path: "skills/"` and dedupes by repo. Keeping a wrong publish instruction invited contributors to open a pointless fork-and-PR against this same repository. |
